...
line (e.g. a `"[C#m]verse-line [B]"` chorus row) never reach `PlaceChords` as a
*row* and are harmless — which is why, in the affected capo tab, only the
standalone `Confess` token on its own line fired.
