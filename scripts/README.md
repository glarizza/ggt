...
(both track *tab websites*, not the chopper's internals). `ggt` graduates
...
scripts/clean_tab.py      in.txt   ->  in.cleaned.txt        strip web / tab noise
         |
ggt chopper in.cleaned.txt --key K --capo N --time T --title "..." -o out.chopper
         |
 out.chopper   --imported into BandHelper
```
...
Strip the noise that surrounds a web-downloaded tab so `ggt chopper` gets a
clean chord-over-lyric body. Song metadata is *not* written into the body —
it is passed to `ggt chopper` as flags. `clean` only *removes* things.
...
chopper's regexes accept it (with length/lowercase guards so real words are
never mistaken for chords). If the two ever drift, chopper's tests are the
...
- `output/` (gitignored) holds cleaned tabs and generated `.chopper` files
